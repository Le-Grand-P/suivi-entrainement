...
> **Correctif important (puissance estimée) — réimporte tes sorties.**
> Le facteur de correction du modèle de puissance était appliqué à la
> vitesse *avant* le calcul de la traînée aérodynamique, qui dépend du cube
> de la vitesse — un correctif de ~19% se retrouvait amplifié jusqu'à +205%
> en descente rapide, et la puissance était surestimée sur tout ce qui
> n'est pas une montée lente. Corrigé : le facteur s'applique maintenant à
> la puissance finale, uniformément. Sur une sortie de test, le %FTP est
> passé de 123% (physiologiquement impossible sur 3h) à 96%. **Seules les
> sorties réimportées après ce correctif ont une puissance/TSS/%FTP
> exacts** — celles déjà en base gardent l'ancienne valeur (trop haute)
> jusqu'à réimport.
